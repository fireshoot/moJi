...
*/
    ...
    public List<TbPanel> getHome() {
        TbPanel build = TbPanel.builder().id(1)
                ...
                .position(1)
                .updated(new Date())
                .panelContents(Collections.emptyList()).build();
...
/*
    ...
    @Override
    public ResultPageView<Product> getAllProduct(int page, int size, String sort, Long cid, int priceGt, int priceLte) {
        Product product = new Product();
        product.setProductId(1001L);
        product.setProductName("测试测试");
        product.setSalePrice(BigDecimal.ONE);
        product.setProductImageBig("https://exp-picture.cdn.bcebos.com/e177fc9147e833e092912fb630ea3e8631485978.jpg?x-bce-process=image%2Fresize%2Cm_lfit%2Cw_500%2Climit_1%2Fformat%2Cf_jpg%2Fquality%2Cq_80");
        product.setSubTitle("hhh");
        return new ResultPageView(1L, 1, 1, Collections.singletonList(product));
